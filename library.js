...
DiscordAuth.filterAccount = function (payload, callback) {
    // console.log(JSON.safeStringify(payload.templateData));
    User.getUserField(payload.templateData.uid, constants.name + 'Id', function (err, did) {
        if (err) return callback(err)
        if (did) {
            client.users.fetch(did).then(fetched => {
                payload.templateData.discordData = {
                    id: did,
